...
static int go_bio_puts(BIO *b, const char *str)
{
    return go_bio_write(b, (char *) str, (int) strlen(str));
}
...
int X_EVP_DigestSignInit(EVP_MD_CTX *ctx,
                         EVP_PKEY_CTX **pctx,
                         const EVP_MD *type,
                         ENGINE *e,
                         EVP_PKEY *pkey)
...
int X_EVP_DigestSign(EVP_MD_CTX *ctx,
...
int X_EVP_DigestVerifyInit(EVP_MD_CTX *ctx,
                           EVP_PKEY_CTX **pctx,
                           const EVP_MD *type,
                           ENGINE *e,
                           EVP_PKEY *pkey)
...
int X_EVP_DigestVerify(EVP_MD_CTX *ctx,
...
{
    go_bio = BIO_meth_new(BIO_TYPE_SOURCE_SINK, "Go Write BIO");
    if (!go_bio)
    {
        return 1;
    }
    if (1 != BIO_meth_set_write(
               go_bio, (int (*)(BIO *, const char *, int)) go_bio_write))
    {
        return 2;
    }
    if (1 != BIO_meth_set_puts(go_bio, go_bio_puts))
    {
        return 3;
    }
    if (1 != BIO_meth_set_ctrl(go_bio, go_bio_ctrl))
    {
        return 4;
    }
    if (1 != BIO_meth_set_create(go_bio, x_bio_create))
    {
        return 5;
    }
    if (1 != BIO_meth_set_destroy(go_bio, x_bio_free))
    {
        return 6;
    }
    if (1 != BIO_meth_set_read(go_bio, go_bio_read))
    {
        return 8;
    }
...
int X_PEM_write_bio_PrivateKey_traditional(BIO *bio,
                                           EVP_PKEY *key,
                                           ...
                                           void *u)
{
    return PEM_write_bio_PrivateKey_traditional(
      bio, key, enc, kstr, klen, cb, u);
}
...
static BIO_METHOD go_bio = { BIO_TYPE_SOURCE_SINK,
                             "Go BIO",
                             (int (*)(BIO *, const char *, int)) go_bio_write,
                             go_bio_read,
                             go_bio_puts,
                             NULL,
                             go_bio_ctrl,
                                           ...
                                           void *u)
{
     ...
     * use PEM_ASN1_write_bio directly to write the DER encoding with the
     * correct type header. */
    ...
    if (EVP_PKEY_asn1_get0_info(&ppkey_id,
                                &pkey_base_id,
                                &ppkey_flags,
                                &pinfo,
                                &ppem_str,
...
long X_SSL_CTX_set_tlsext_servername_callback(SSL_CTX *ctx,
                                              int (*cb)(SSL *con,
                                                        int *ad,
                                                        void *args))
{
...
int X_SSL_CTX_set_tlsext_ticket_key_cb(
  SSL_CTX *sslctx,
  int (*cb)(SSL *s,
...
int X_SSL_CTX_ticket_key_cb(SSL *s,
                            unsigned char key_name[16],
                            unsigned char iv[EVP_MAX_IV_LENGTH],
                            EVP_CIPHER_CTX *cctx,
                            HMAC_CTX *hctx,
...
BIO *X_BIO_new_go_bio()
{
    return BIO_new(BIO_s_go());
}
...
int X_EVP_SignFinal(EVP_MD_CTX *ctx,
                    unsigned char *md,
                    unsigned int *s,
                    EVP_PKEY *pkey)
{
...
int X_EVP_VerifyFinal(EVP_MD_CTX *ctx,
...
int X_HMAC_Init_ex(HMAC_CTX *ctx,
                   const void *key,
                   int len,
                   const EVP_MD *md,
                   ENGINE *impl)
